...
interface Landmark { name: string; distance: string; type: string }
interface Project {
  ...
  carpet_area_min: number | null; carpet_area_max: number | null
  usps: string[]; landmarks: Landmark[]; pitch_script: string | null
  google_maps_url: string | null; rera_number: string | null; tags: string[] | null
}
interface Props { projectId: string; user: any; onBack: () => void; onLogout: () => void }
...
export default function ProjectPage({ projectId, user, onBack, onLogout }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('whatsapp_templates').select('message').eq('salesperson_id', user.id).eq('project_id', projectId).maybeSingle()
    ]).then(([{ data: p }, { data: s }, { data: t }]) => {
      setProject(p as Project || null)
      setSimilar(s as Project[] || [])
      setWaTemplate(t?.message || null)
      setLoading(false)
  ...
  const sendWA = () => {
    if (!project || phone.length !== 10 || !waTemplate) return
    const msg = encodeURIComponent(waTemplate)
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: 24, width: '100%', display: 'grid', gridTemplateColumns: '1fr 280px', gap: 24, alignItems: 'start', boxSizing: 'border-box' }}>
        ...
        <div>
          ...
          <div style={{ height: 240, borderRadius: 14, overflow: 'hidden', background: 'linear-gradient(135deg,rgba(79,70,229,0.3),rgba(147,51,234,0.3))', marginBottom: 20, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {project.image_url ? <img src={project.image_url} alt={project.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 64, opacity: 0.3 }}>🏢</span>}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 24 }}>
            {[['Price', `${fmt(project.price_min)} – ${fmt(project.price_max)}`], ['BHK', project.bhk_types?.join(', ') || '—'], ['Carpet Area', project.carpet_area_min ? `${project.carpet_area_min}–${project.carpet_area_max} sqft` : '—'], ['Possession', project.possession_date]].map(([l, v]) => (
          ...
          {tab === 'overview' && (
            <div>
              <div style={{ fontSize: 15, fontWeight: 600, color: '#A5B4FC', marginBottom: 12 }}>Key Highlights</div>
              {project.usps?.filter(Boolean).length > 0
                ? <ul style={{ listStyle: 'none', padding: 0, marginBottom: 24 }}>
                    {project.usps.filter(Boolean).map((u, i) => (
                      <li key={i} style={{ display: 'flex', gap: 10, marginBottom: 10, fontSize: 15, color: '#E2E8F0' }}>
              ...
              {project.bhk_types?.length > 0 && (
                <div style={{ borderRadius: 10, overflow: 'hidden', border: '1px solid rgba(79,70,229,0.2)' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                    <thead><tr style={{ background: 'rgba(79,70,229,0.2)' }}>
                      {['Type','Carpet Area','Price Range'].map(h => <th key={h} style={{ padding: '10px 14px', textAlign: 'left', color: '#A5B4FC' }}>{h}</th>)}
                    </tr></thead>
                    <tbody>
                      {project.bhk_types.map((b, i) => (
                        <tr key={i} style={{ borderTop: '1px solid rgba(79,70,229,0.15)' }}>
                          <td style={{ padding: '10px 14px', fontWeight: 500 }}>{b}</td>
                          <td style={{ padding: '10px 14px', color: '#94A3B8' }}>{project.carpet_area_min ? `${project.carpet_area_min}–${project.carpet_area_max} sqft` : '—'}</td>
                          <td style={{ padding: '10px 14px', color: '#A5B4FC' }}>{fmt(project.price_min)} – {fmt(project.price_max)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              : <div>
                  {lms.map((lm, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(79,70,229,0.1)', borderRadius: 8, padding: '12px 14px', marginBottom: 8, border: '1px solid rgba(79,70,229,0.2)' }}>
                      <div>
                        <div style={{ fontSize: 15, marginBottom: 4 }}>{lm.name}</div>
                        <span style={{ fontSize: 11, background: 'rgba(99,102,241,0.25)', color: '#A5B4FC', padding: '2px 8px', borderRadius: 10 }}>{lm.type}</span>
                      </div>
                      <div style={{ color: '#6366F1', fontWeight: 700, fontSize: 14 }}>{lm.distance}</div>
                    </div>
                  ))}
                </div>
          )}
            ...
            project.pitch_script
              ? <div style={{ background: 'rgba(79,70,229,0.1)', border: '1px solid rgba(79,70,229,0.3)', borderRadius: 12, padding: 22 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                    <div style={{ fontSize: 15, fontWeight: 600, color: '#A5B4FC' }}>Pitch Script</div>
                    <span style={{ fontSize: 11, background: 'rgba(16,185,129,0.2)', color: '#10B981', padding: '3px 10px', borderRadius: 20 }}>Script Ready ✓</span>
                  </div>
                  <p style={{ color: '#E2E8F0', lineHeight: 1.9, fontSize: 15, marginBottom: 18, whiteSpace: 'pre-line' }}>{project.pitch_script}</p>
                  <button onClick={() => { navigator.clipboard.writeText(project.pitch_script!); setCopied(true); setTimeout(() => setCopied(false), 2000) }} style={{ background: 'rgba(99,102,241,0.2)', border: '1px solid rgba(99,102,241,0.4)', borderRadius: 7, padding: '8px 18px', color: '#A5B4FC', cursor: 'pointer', fontSize: 14 }}>
                    {copied ? '✅ Copied!' : '📋 Copy Script'}
                  </button>
                </div>
              : <div style={{ textAlign: 'center', padding: 40, color: '#64748B' }}>No pitch script added yet.</div>
          )}

          {/* Similar projects */}
          {similar.length > 0 && (
            <div style={{ marginTop: 36 }}>
              <div style={{ fontSize: 15, fontWeight: 600, color: '#A5B4FC', marginBottom: 14 }}>Similar Projects</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
                {similar.map(s => (
                  <div key={s.id} style={{ background: '#1E1B4B', borderRadius: 10, padding: 14, border: '1px solid rgba(79,70,229,0.2)' }}>
                    <div style={{ fontSize: 11, color: '#6366F1', marginBottom: 3 }}>{s.developer}</div>
                    <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 4 }}>{s.name}</div>
                    <div style={{ fontSize: 13, color: '#94A3B8', marginBottom: 8 }}>📍 {s.location}</div>
                    <div style={{ fontSize: 14, color: '#A5B4FC', fontWeight: 600 }}>{fmt(s.price_min)} – {fmt(s.price_max)}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{ position: 'sticky', top: 72 }}>
          ...
          <div style={{ background: '#1E1B4B', border: '1px solid rgba(79,70,229,0.3)', borderRadius: 12, padding: 18, marginBottom: 14 }}>
            <div style={{ fontSize: 26, fontWeight: 700, background: 'linear-gradient(90deg,#818CF8,#A78BFA)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', marginBottom: 4 }}>{fmt(project.price_min)}</div>
            <div style={{ fontSize: 13, color: '#64748B', marginBottom: 14 }}>Starting price onwards</div>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {project.bhk_types?.map(b => <span key={b} style={{ background: 'rgba(99,102,241,0.2)', color: '#A5B4FC', fontSize: 12, padding: '3px 9px', borderRadius: 4 }}>{b}</span>)}
            </div>
          </div>

          {/* WhatsApp */}
          <div style={{ background: '#1E1B4B', border: '1px solid rgba(79,70,229,0.3)', borderRadius: 12, padding: 18, marginBottom: 14 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: '#A5B4FC', marginBottom: 12 }}>📲 Send to Client</div>

            {templateLoading ? (
              <div style={{ fontSize: 13, color: '#64748B', textAlign: 'center', padding: 10 }}>Loading...</div>
            ) : !waTemplate ? (
              <div style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 8, padding: '12px 14px', fontSize: 13, color: '#FCD34D', lineHeight: 1.6 }}>
                ⚠️ WhatsApp message not set for this project.<br/>
                <span style={{ color: '#94A3B8', fontSize: 12 }}>Click your avatar (top right) → set message for this project.</span>
              </div>
            ) : (
              <>
                <div style={{ background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 8, padding: '8px 12px', fontSize: 12, color: '#10B981', marginBottom: 12 }}>
                ...
                <input
                  ...
                  style={{ width: '100%', background: 'rgba(79,70,229,0.12)', border: '1px solid rgba(79,70,229,0.3)', borderRadius: 8, padding: '9px 12px', color: 'white', fontSize: 14, outline: 'none', marginBottom: 10, boxSizing: 'border-box' }}
                />
                <button onClick={sendWA} disabled={phone.length !== 10}
                  style={{ width: '100%', padding: 11, borderRadius: 8, border: 'none', background: phone.length === 10 ? '#25D366' : 'rgba(37,211,102,0.2)', color: phone.length === 10 ? 'white' : '#64748B', fontWeight: 700, cursor: phone.length === 10 ? 'pointer' : 'default', fontSize: 15 }}>
                  💬 Send on WhatsApp
                </button>
              </>
            )}
          ...
          {project.google_maps_url && (
            <a href={project.google_maps_url} target="_blank" rel="noreferrer"
              style={{ display: 'block', background: 'rgba(79,70,229,0.15)', border: '1px solid rgba(79,70,229,0.3)', borderRadius: 10, padding: 12, color: '#A5B4FC', textDecoration: 'none', textAlign: 'center', fontSize: 14 }}>
