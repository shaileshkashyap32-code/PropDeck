    ...
    <div>
      ...
      <div className="ds" style={{ position: 'relative', height: 20 }}>
        <input
          ...
          value={vMin}
          onChange={(e) =>
            onChange(Math.min(+e.target.value, vMax - STEP), vMax)
          }
        />
        <input
          type="range"
          ...
          value={vMax}
          onChange={(e) =>
            onChange(vMin, Math.max(+e.target.value, vMin + STEP))
          }
        />
      </div>
    </div>
...
export default function Home({
  ...
  const toggleType = (b: string) =>
    setSelType((p) => (p.includes(b) ? p.filter((x) => x !== b) : [...p, b]));
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        >
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            >
              ...
              {LOCS.map((l) => (
                <option key={l} value={l} style={{ background: '#1E1B4B' }}>
                  {l}
                </option>
              ))}
            </select>
                    ...
                    style={{
                      ...
                      background: on
                        ? isPlot
                          ...
                          : 'rgba(99,102,241,0.3)'
                        : 'transparent',
                      color: on ? (isPlot ? '#10B981' : '#A5B4FC') : '#64748B',
                    }}
                  >
                    {isPlot ? '🏞️ Plot' : b}
                  </button>
        ...
        <main style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
          ...
          {loading ? (
            ...
            >
              {shown.map((p) => (
                <Card key={p.id} project={p} onView={onViewProject} />
              ))}
            </div>
          )}
        </main>
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 14,
        border: `1px solid ${
          isPlot ? 'rgba(16,185,129,0.3)' : 'rgba(79,70,229,0.2)'
        }`,
        overflow: 'hidden',
        cursor: 'pointer',
      ...
      >
        ...
        <span
          style={{
            ...
            padding: '3px 10px',
            borderRadius: 20,
            background: p.status === 'Ready to Move' ? '#10B981' : '#F59E0B',
            color: 'white',
          }}
        >
